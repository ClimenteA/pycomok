...
class Mail:
    """
    Send emails with Dispatch using win32com.client !
    
        Bellow is the basic usage:    
    ...
    def get_email_address(self, mail_item):
    ...
    def process_filter_dates(self, starting_from_date, until_date): 
        """
    ...
    def filter_items_by_date(self, mail_items, starting_from_date=None, until_date=None):
        """
        ...
        starting_from_date, until_date = self.process_filter_dates(starting_from_date, until_date)
    ...
    def get_item_recipients(self, mail_item):
        """
        ...
        for recipient in mail_item.Recipients:
            receiver = {
                "name": recipient.name,
                "email": self.get_email_address(recipient)
            } 
    ...
    def get_items_data(self, mail_items, save_mail_item=False):
        """
        ...
        for item in mail_items:
            ...
            item_data = {
                     ...
                     "sender": {"name": item.SenderName, "email": self.get_email_address(item)},
                     "receivers": self.get_item_recipients(item),
    ...
    def get_mail_item(self, mail_items):
        """
    ...
    def get_accounts(self):
        ...
        """
        accounts_emails = [acc.SmtpAddress for acc in self.outlook.Accounts]
        accounts_names = [acc.CurrentUser.name for acc in self.outlook.Accounts]
    ...
    def get_outlook_mail_items(self, account_email=None, outlook_folder_path=None, display_accounts=False):
        """
        ...
        #it's used in com object
        account_folders = self.outlook.Folders.Item(account_email) 
    ...
    def send_email(self, subject, message, to, cc=None, attachments=[], display=True, send=True, html=True):
        """
        ...
        if html:
            new_mail.HTMLBody = str(message)
        else:
            new_mail.Body = str(message)
            
    
        new_mail.To = self.get_to_cc(to)
        
        if cc:
            new_mail.CC = self.get_to_cc(cc)
